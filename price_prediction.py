...
class PricePredictionModel:
    ...
    def load_memory(self, asset):
        try:
            ...
            return np.frombuffer(row[0]["compressed_memory"], dtype=np.float32).reshape(SEQUENCE_LENGTH, 1)
    ...
    def save_memory(self, asset, new_data):
        ...
        if not existing.is_empty():
            if np.array_equal(np.frombuffer(existing[0]["compressed_memory"], dtype=np.float32), compressed):
                return
            self.memory_df = self.memory_df.filter(pl.col("asset") != asset)

        new_row = pl.DataFrame({"asset": [asset], "compressed_memory": [compressed.tobytes()]})
    ...
    def train_model(self, asset, raw_data):
        ...
        extra_features = np.concatenate([[signal_score], emb_m1, emb_m5, emb_m15, emb_m30, emb_1h, emb_4h, emb_1d])
        ...
        local_model.fit(full_input, y, epochs=10, batch_size=BATCH_SIZE, verbose=1, callbacks=[early_stop])
    ...
    def predict_price(self, asset: str, full_state: np.ndarray = None) -> float:
        ...
        if full_state is not None:
            try:
                ...
                logging.error(f"❌ Errore durante la previsione con full_state per {asset}: {e}")
                return None
