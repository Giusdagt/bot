...
drl_agent.DRLAgent = type('DummyDRLAgent', (object,), {
    "__init__": lambda self, state_size=None: setattr(self, "state_size", state_size) or None,
...
# Righe 336-340
df_asset1 = pl.DataFrame(data_asset1)
df_asset2 = pl.DataFrame(data_asset2)
...
print(f"Market data map: {market_data_map}")
print(f"Account balances: {ai_model.fetch_account_balances()}")
...
try:
    system = ai_model.AIModel(market_data_map, ai_model.fetch_account_balances())
    print("Initialization: SUCCESS - AIModel and components initialized.")
except Exception as e:
    print(f"Initialization: FAILURE - Exception occurred: {e}")
    logging.exception("Dettagli dell'errore durante l'inizializzazione:")
    system = None
...
if system:
    try:
        assets_selected = system.active_assets
...
async def run_decisions():
    for asset in system.active_assets:
        print(f"\n### Deciding trade for {asset} ###")
        try:
            result = await system.decide_trade(asset)
            if result is False:
                print(
