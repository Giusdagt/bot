...
class DRLSuperAgent:
    ...
    def train(self, steps=5000):
        ...
        try:
            ...
            return
        model_path = MODEL_PATH / f"{self.algo}_model"
    ...
    def predict(self, state):
        if np.isnan(state).any():
            logging.error("❌ Stato contiene NaN. Applico correzione preventiva.")
            state = np.nan_to_num(state, nan=0.0, posinf=1.0, neginf=-1.0)
        # Check preventivo per robustezza
        if state.ndim != 2 or state.shape[1] == 0:
             raise ValueError("⚠️ Stato non valido, verifica i dati in ingresso.")
        if USE_DYNAMIC_STATE_SIZE:
            # Modalità Adattativa (con PCA o padding dinamico)
            if state.shape[1] > DESIRED_STATE_SIZE:
                from sklearn.decomposition import PCA
                if state.shape[0] >= DESIRED_STATE_SIZE:
                    pca = PCA(n_components=DESIRED_STATE_SIZE)
                    state = pca.fit_transform(state).reshape(state.shape[0], -1)
                else:
                    logging.warning("⚠️ PCA non applicato: dati insufficienti, uso padding.")
    ...
    def load(self):
        ...
        da file salvati.
        """
        self.model = self.model.load(str(MODEL_PATH / f"{self.algo}_model"))
        self.drl_agent.load()
